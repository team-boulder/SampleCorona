...
function M.saveTable(t, filename, location)
    ...
    if file then
        ...
        print("WARN: can't save: " .. filename)
        return false
    end
end

function M.loadTable(filename, location)
    if location and (not _validLoadLocations[location]) then
...
function M.removeFile(filename, location)
    if location and (not _validLoadLocations[location]) then
        error("Attempted to load a table from an invalid location", 2)
    elseif not location then
        location = _defaultLocation
    ...
    local result, reason = os.remove( system.pathForFile( filename, location) )
    return result
end

function M.loadTableOrInit(dir, filename)
    local rawData = M.loadTable(filename)
    if not rawData then
        rawData = M.loadTable(dir .. filename, system.ResourceDirectory)
        M.saveTable(rawData, filename)
    end
    return rawData
end
