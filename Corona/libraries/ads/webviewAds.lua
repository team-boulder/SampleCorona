...
function this.prepare(ads_type, option)
	...
	if ads[ads_type]['url'] then
		if ads[ads_type] == nil then return end
		ads[ads_type]['prepared'] = true
		ads[ads_type]['view'] = native.newWebView(_W, _H, w or _W, h or 100)
		ads[ads_type]['view'].x, ads[ads_type]['view'].y = _W*2.5, _H*2.5
		...
		ads[ads_type]['view']:request(ads[ads_type]['url'], system.ResourceDirectory)
...
function this.show(ads_type, option)
	...
	if not ads_type then return end
	...
	local function showAds()
		...
		if view == true then
			...
			ads[ads_type]['view'].width, ads[ads_type]['view'].height = w, h
	...
	if not ads[ads_type]['url'] then
		...
		this:addEventListener( 'webviewAds', showListener )
...
function this.unreveal( ads_type )
	...
	if ads[ads_type] and ads[ads_type]['view'] then
		...
		if target then
			if ads[ads_type]['view'].cached_x == ads[ads_type]['view'].x or ads[ads_type]['view'].cached_y == ads[ads_type]['view'].y then return -1 end
			if ads[ads_type]['view'].x == _W*2 or ads[ads_type]['view'].y == _H*2 then return -1 end
			ads[ads_type]['view'].cached_x, ads[ads_type]['view'].cached_y = ads[ads_type]['view'].x, ads[ads_type]['view'].y
			ads[ads_type]['view'].x, ads[ads_type]['view'].y = ads[ads_type]['view'].x+_W*2, ads[ads_type]['view'].y+_H*2
			print( ads[ads_type]['view'].x, ads[ads_type]['view'].y )
		end
	end
end

function this.reveal( ads_type, options )
	assert(ads_type, 'ERROR : ads_typeを指定して下さい')
	if ads[ads_type] and ads[ads_type]['view'] then
		local target = ads[ads_type]['view']
		if target then
			print( options )
			local o = options or { x = target.x, y = target.y, width = target.width, height = target.height }
			target.width, target.height = o.width, o.height
			target.x, target.y = o.x+target.width*0.5, o.y+target.height*0.5
...
function this.showRewardedVideo(url)
	...
	if system.getInfo('platformName') == 'iPhone OS' or system.getInfo('platformName') == 'Android' then
		...
		-- テキストファイルに保存する
		LoadSave.saveTable({sessionID}, 'session_id.txt')
...
function this.endedRewardedVideo(url)
	...
	if url then
		...
		local data = LoadSave.saveTable("session_id.txt")
		local lua_sessionID = data[1]
		...
		if lua_hash == hash then
			...
			-- テキストファイルの削除
			LoadSave.saveTable( "session_id.txt" )
		else
			-- 不正なID
			event.name = 'failed'
		end
