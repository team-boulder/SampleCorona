...
local function adChartboostListener(event)
	...
	elseif event.phase == 'failed' then
		...
		event.phase = 'reward-failed'
	end

	this:dispatchEvent( {name='adnetwork', phase = event.phase } )
end
...
function this.prepare( ads_type, listener )
	...
	if is_initialized then
		...
		timer.performWithDelay( 1000, function()
			this.prepare( ads_type, listener )
		end )
	end
end
...
function this.show( ads_type )
	...
	if SkipRewardVideo == true and ads_type == 'rewardedVideo' then
		adChartboostListener( { phase = 'reward' } )
	else
		if is_initialized then
			if is_loaded then
