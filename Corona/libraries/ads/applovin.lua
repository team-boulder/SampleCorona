...
local function adApplovinListener( event )
	...
	local e = {name='adnetwork', phase = event.response } 
...
function this.init( option )
	...
	print( option )
	print("applovin initialized")
	applovin.init( adApplovinListener, { sdkKey = appID, verboseLogging = true } )
...
function this.show( ads_type )
	...
	if SkipRewardVideo == true and ads_type == 'rewardedVideo' then
		adApplovinListener( { type = 'incentivizedInterstitial', phase = 'playbackEnded' } )
